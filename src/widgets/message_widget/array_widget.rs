...
impl<'a> ArrayWidget<'a> {
    ...
    pub fn selection_height(&self, width: u16) -> u16 {
        ...
        match &self.value {
            ...
            ArrayField::String(_)
            | ArrayField::WString(_)
            | ArrayField::BoundedString(_)
            | ArrayField::BoundedWString(_) => {
                // 1 for the header line, then one line per element up to and including the selected one
                1 + selected_index as u16 + 1
            }
            _ => {
                // Primitive elements are packed ARRAY_ELEMENT_WIDTH columns each
                let elems_per_row = width / ARRAY_ELEMENT_WIDTH;
                if elems_per_row == 0 {
                    1 + selected_index as u16 + 1
                } else {
